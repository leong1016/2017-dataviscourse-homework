...
class ShiftChart {
    ...
    update(selectedStates, isYear){
                        ...
                        if (states[i] == stateData.State) {
                            ...
                            dotgroup.append("circle")
                                ...
                                .on("mouseout", function () {
                                    unhighlight(d3.select(this).attr("id"));
        ...
        function highlight (name) {
            d3.select("#dotgroup").selectAll("#"+name)
                .attr("r", "15px")
            d3.select("#linegroup").selectAll("#"+name)
                .attr("stroke-width", "10px")
        };

        function unhighlight (name) {
            d3.select("#dotgroup").selectAll("circle")
                .attr("r", "10px")
            d3.select("#linegroup").selectAll("line")
                .attr("stroke-width", "5px")
        };
    };
}
